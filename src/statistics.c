    ...
    if (stats_n == NSTATS) {
      ...
      for (j = 0; j < stats_n - 1; j += 2) { /* FIXED IN  0.84.01 */
        for (i = 0; i < stats_np; i++) {
          ...
          stats[k].nplanets[i] =
              (stats[j].nplanets[i] + stats[j + 1].nplanets[i]) / 2;
        }
        k++;
      }
      stats_n /= 2;
...
void PrintStatistics(void) {
  ...
  for (i = 1; i < stats_np; i++) {
    printf("%d: %.1f %d \n", i, stats[NSTATS - 1].level[i],
           stats[NSTATS - 1].nplanets[i]);
  }
  printf("\n");
}
...
void fprintStatistics(FILE* fp) {
  ...
  fprintf(fp, "%d %d %d %d %d ", lasttime, inctime, stats_n, stats_np,
          stats_cont);
...
void fscanfStatistics(FILE* fp) {
  ...
  if (fscanf(fp, "%d%d%d%d%d", &lasttime, &inctime, &stats_n, &stats_np,
             &stats_cont) != 5) {
    perror("fscanf");
    exit(-1);
  }
  ...
  for (i = 0; i < NSTATS; i++) {
    for (j = 0; j < stats_np; j++) {
      ...
      if (stats[i].level[j] < 0) {
        fprintf(stderr,
                "ERROR fscanfStatistics(): level <0. Setting to zero\n");
        stats[i].level[j] = 0;
      }
      ...
      if (stats[i].nplanets[j] < 0) {
        fprintf(stderr,
                "ERROR fscanfStatistics(): nplanets <0. Setting to zero\n");
