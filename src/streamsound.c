...
struct StreamedSound* StreamSound(char* filename, int mode) {
  ...
  for (i = 0; i < NUM_BUFFER_MUSIC; i++) {
    ...
    alBufferData(streambuffers[i], format, dec_music, BUFFER_MUSIC_SIZE,
                 information->rate);
    if (alGetError() != AL_NO_ERROR) {
      reportError();
    }
  ...
  while (arg->state != SS_STOP) {
    ...
    if (procesed_buffers > 0) {
      while (procesed_buffers) {
        ...
        bytes = ov_read((arg->ogg_file), (char*)dec_music, buffersize, 0, 2, 1,
                        &current);
        if (alGetError() != AL_NO_ERROR) {
          reportError();
          break;
        ...
        if (bytes == 0) { /* EOF */
          if (arg->mode == SM_LOOP) {
            ...
            bytes = ov_read((arg->ogg_file), (char*)dec_music, buffersize, 0, 2,
                            1, &current);
            arg->state = SS_PLAY;
          } else {
            arg->state = SS_PAUSE;
