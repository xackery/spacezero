...
void DrawText(GdkDrawable* pixmap,
              GdkFont* font,
              GdkGC* gc,
              gint x,
              gint y,
              const gchar* string);
void DrawString_1(GdkDrawable* pixmap,
                  GdkFont* font,
                  GdkGC* gc,
                  gint x,
                  gint y,
...
GtkWidget* InitGraphics(char* title,
                        char* optfile,
                        int w,
                        int h,
  ...
  gtk_window_set_geometry_hints(GTK_WINDOW(win_main), GTK_WIDGET(d_a),
                                &geometry,
  ...
  gtk_widget_set_events(win_main, GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK |
                                      GDK_KEY_PRESS_MASK |
  ...
  gtk_widget_set_events(
      d_a, GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_KEY_PRESS_MASK |
               GDK_KEY_RELEASE_MASK | GDK_POINTER_MOTION_MASK |
  ...
  gtk_signal_connect(GTK_OBJECT(about2), "clicked", GTK_SIGNAL_FUNC(QuitWindow),
                     winabout);
  ...
  strcat(labelhelp, "left mouse \t Select the nearest ship.\nbutton\n");
  strcat(labelhelp,
  ...
  gtk_signal_connect(GTK_OBJECT(help2), "clicked", GTK_SIGNAL_FUNC(QuitWindow),
                     winhelp);
  ...
  options1 = gtk_check_button_new_with_label(
      GetLocale(L_KNOWNUNIVERSE)); /* "Known Universe" */
  gtk_widget_show(options1);
  gtk_signal_connect(GTK_OBJECT(options1), "toggled",
                     GTK_SIGNAL_FUNC(PrintMessage), "option1");

  options2 = gtk_check_button_new_with_label(
      GetLocale(L_MUSICOFF)); /* "Music OFF"); */
  gtk_widget_show(options2);
  gtk_signal_connect(GTK_OBJECT(options2), "toggled",
                     GTK_SIGNAL_FUNC(PrintMessage), "option2");

  options3 =
      gtk_check_button_new_with_label(GetLocale(L_SOUNDOFF)); /* "Sound OFF" */
  ...
  options10 =
      gtk_button_new_with_label(GetLocale(L_SETDEFAULT)); /* " Set Default" */
  ...
  gtk_box_pack_start(GTK_BOX(vbox2), hbox3, FALSE, FALSE,
                     0); /* universe size */
  gtk_box_pack_start(GTK_BOX(vbox2), hbox5, FALSE, FALSE,
  ...
  gtk_box_pack_start(GTK_BOX(hbox3), options8, FALSE, FALSE,
                     0); /* universe size */
  gtk_box_pack_start(GTK_BOX(hbox3), options9, FALSE, FALSE, 0);

  gtk_box_pack_start(GTK_BOX(hbox5), options17, FALSE, FALSE,
                     0); /* number of galaxies */
  ...
  gtk_box_pack_start(GTK_BOX(hbox4), options12, FALSE, FALSE,
                     0); /*quit options*/
  /* gtk_box_pack_start(GTK_BOX(vbox2),vbox3,FALSE,FALSE,0);/\* about menu *\/
   * /\* HERE why must be commented*\/ */
  ...
  gtk_tooltips_set_tip(tooltips, options1,
                       "All The Universe is known by all players.", NULL);

  tooltips = gtk_tooltips_new();
  gtk_tooltips_set_tip(tooltips, options14,
                       "All human players belongs to the same team.", NULL);

  tooltips = gtk_tooltips_new();
  gtk_tooltips_set_tip(tooltips, options15,
                       "All computer players belongs to the same team.", NULL);

  tooltips = gtk_tooltips_new();
  gtk_tooltips_set_tip(tooltips, options16,
                       "If the Queen ship is destroyed, all player ships are "
                       "destroyed. GAME OVER.",
                       NULL);
...
gint expose_event(GtkWidget* widget, GdkEventExpose* event) {
  ...
  gdk_draw_pixmap(
      ...
      event->area.x, event->area.y, event->area.x, event->area.y, -1, -1);
...
gint configure_event(GtkWidget* widget, GdkEventConfigure* event) {
  ...
  GameParametres(SET, GHEIGHT,
                 widget->allocation.height - GameParametres(GET, GPANEL, 0));
  GameParametres(SET, GWIDTH, widget->allocation.width);

  if (pixmap_stat) {
    gdk_pixmap_unref(pixmap_stat);
  }
  pixmap_stat = gdk_pixmap_new(widget->window, 200, 100,
...
int DrawObjs(GdkPixmap* pixmap,
             struct HeadObjList* lhc,
  ...
  */
  ...
  while (ls != NULL) {
    ...
    if (ls->obj->habitat != habitat.type) {
      ls = ls->next;
      continue;
    }
    if (cv->in != ls->obj->in) {
    ...
    switch (ls->obj->type) {
        ...
        switch (ls->obj->subtype) {
          ...
          case MISSILE: /*SHOT3: */
            ...
            for (i = 0; i < missile[0] - 1; i++) {
              ...
              gdk_draw_line(pixmap, gc, x + x_0, gheight - (y + y_0), x + x_1,
                            gheight - (y + y_1));
            }
            ...
            if (ls->obj->accel > 0) {
              ...
              gdk_draw_line(pixmap, penCyan, x + x_0, gheight - (y + y_0),
                            x + x_2, gheight - (y + y_2));
              gdk_draw_line(pixmap, penCyan, x + x_1, gheight - (y + y_1),
                            x + x_2, gheight - (y + y_2));
            }
            /********** --engine flares *************/
          ...
          case LASER: /*SHOT4: */
            ...
            gdk_draw_line(pixmap, penBlue, x0, gheight - (y0), x1,
                          gheight - (y1));
            break;
          case EXPLOSION:
      ...
      case SHIP:
        switch (ls->obj->subtype) {
          ...
          case TOWER:
          case PILOT:
            if (ls->obj->habitat == H_PLANET && ls->obj == cv &&
              ...
              gc = gcolors[players[ls->obj->player].color];
              if (ls->obj->selected == TRUE && ls->obj != cv &&
                  ls->obj->player == cv->player)
                gc = penGrey;
      ...
      default:
        g_print("ERROR DrawObjs(2)%d %d %d %d %d\n", ls->obj->id, ls->obj->type,
                ls->obj->subtype, ls->obj->player, n);
        /* exit(-1); */
        break;
    }
  ...
  */
  ...
  switch (obj->subtype) {
    case PILOT:
      ...
      gdk_draw_rectangle(pixmap, gc2, TRUE, x - 1, gheight - y - 1, 3, 3);
    ...
    case GOODS:
      ...
      if (obj->accel > 0) {
        ...
        gdk_draw_line(pixmap, penCyan, x + x_0, gheight - (y + y_0), x + x_2,
                      gheight - (y + y_2));
        gdk_draw_line(pixmap, penCyan, x + x_1, gheight - (y + y_1), x + x_2,
                      gheight - (y + y_2));
      }
      /********** --engine flares *************/
    ...
    case SATELLITE:
      gdk_draw_arc(pixmap, gc, FALSE, x - obj->radio, gheight - y - obj->radio,
                   2 * obj->radio, 2 * obj->radio, 0, 23040);
      break;

    case TOWER:
      if (obj->player != ppirates) {
        gdk_draw_rectangle(pixmap, gc, FALSE, x - .5 * obj->radio,
                           gheight - y - obj->radio, obj->radio,
                           2 * obj->radio);
      } else {
        x0 = x - 0.5 * obj->radio;
...
void DrawPlanet(GdkPixmap* pixmap, int x, int y, int r) {
  ...
  */
  ...
  gdk_draw_arc(pixmap, penBlue, TRUE, x - r, gheight - y - r, 2 * r, 2 * r, 0,
               23040);
...
void DrawStars(GdkPixmap* pixmap, int mode, float rx, float ry) {
  ...
  */
  ...
  if (mode == RELATIVE) {
    ...
    for (i = 0; i < numstars; i++) {
      gdk_draw_point(pixmap, pen[stars.color[i]], stars.x[i],
                     gheight - stars.y[i]);
    }
  }
  sw2++;
...
void DrawPlanetSurface(GdkPixmap* pixmap, struct Planet* planet, GdkGC* color) {
  ...
  */
  ...
  while (s != NULL) {
    ...
    gdk_draw_line(pixmap, gc, sx * s->x0, height - sy * s->y0, sx * s->x1,
                  height - sy * s->y1);

    if (s->type == LANDZONE) {
      gdk_draw_line(pixmap, gc, sx * s->x0, height - sy * s->y0 + 1, sx * s->x1,
                    height - sy * s->y1 + 1);
      gdk_draw_line(pixmap, gc, sx * s->x0, height - sy * s->y0 + 2, sx * s->x1,
                    height - sy * s->y1 + 2);
    }
    s = s->next;
  }
...
int DrawRadar(GdkPixmap* pixmap,
              Object* obj,
              struct HeadObjList* lhc,
              int crash) {
  /*
  ...
  */
  ...
  if (!crash) {
    gdk_draw_arc(pixmap, gc, FALSE, x0 - d, gheight - y0 - d, 2 * d, 2 * d, 0,
                 23040);
  } else {
    gdk_draw_arc(pixmap, penSoftRed, TRUE, x0 - d, gheight - y0 - d, 2 * d,
                 2 * d, 0, 23040);
  }
  ...
  while (ls != NULL) {
    ...
    switch (ls->obj->type) {
        ...
        switch (ls->obj->subtype) {
          case PILOT: {
            ...
            if (GetTime() % interval < interval / 2) {
              gdk_draw_line(pixmap, gc, x - 1, gheight - y, x + 1, gheight - y);
              gdk_draw_line(pixmap, gc, x, gheight - y - 1, x, gheight - y + 1);
            }
          } break;
          case SATELLITE:
          case GOODS:
      ...
      case PLANET:
        ...
        gdk_draw_rectangle(pixmap, gc, TRUE, x - 1, gheight - y - 1, 3, 3);
        n++;
        break;
      case ASTEROID:
        gc = penRed;
...
void DrawSpaceShip(GdkPixmap* pixmap, Object* obj, struct HeadObjList* lhc) {
  ...
  */
  ...
  int i, j;
  float fx, fy;
  float s[] = {8,    LINE,  -.75,  1.5,  .75,  1.5,   LINE, .75,   1.5,
               1.25, 1,     LINE,  1.25, 1,    1.25,  .25,  LINE,  1.25,
               .25,  .75,   -.25,  LINE, .75,  -.25,  -.75, -.25,  LINE,
               -.75, -.25,  -1.25, .25,  LINE, -1.25, .25,  -1.25, 1,
               LINE, -1.25, 1,     -.75, 1.5};
  ...
  for (i = 0; i < s[0]; i++) {
    switch ((int)s[j]) {
      ...
      case LINE:
        ...
        y2 = s[++j];
        j++;
        gdk_draw_line(pixmap, gc, x0 + x1 * fx, gheight - (y0 + y1 * fy),
...
void DrawAsteroid(GdkPixmap* pixmap, int x, int y, Object* obj) {
  ...
  */
  ...
  gdk_draw_polygon(pixmap, gc, FALSE, pts, 16);
}

void DrawMap(GdkPixmap* pixmap,
             int player,
             struct HeadObjList hol,
             Object* cv,
             int ulx) {
  ...
  */
  ...
  static struct HeadObjList
      listheadnearobjs; /* list of near enemies objects of actual player.*/
  ...
  if (keys.order.state == FALSE) {
     ...
     */
    /* 		    "O: Introduce command");  */
    ShellTitle(0, NULL, pixmap, penRed, gfont, 10,
               gheight + GameParametres(GET, GPANEL, 0) / 2 + 4);
  ...
  if (GameParametres(GET, GKPLANETS, 0) == FALSE) {
    ...
    if (label & 2) {
      ...
      while (ks != NULL) {
        ...
        if (sd > 3) {
          gdk_draw_line(pixmap, gc, x, gheight - (y), x + sd,
                        gheight - (y + sd));

          gdk_draw_line(pixmap, gc, x, gheight - (y + sd), x + sd,
                        gheight - (y));
        } else {
          gdk_draw_point(pixmap, gc, x, gheight - (y));
        }
      ...
      case SHIP:
        ...
        switch (ls->obj->subtype) {
          case PILOT: {
            ...
            if (GetTime() % interval < interval / 2) {
              gc = penRed;
              gdk_draw_line(pixmap, gc, x - 4, gheight - y, x - 2, gheight - y);
              gdk_draw_line(pixmap, gc, x + 2, gheight - y, x + 4, gheight - y);
              gdk_draw_line(pixmap, gc, x, gheight - y - 4, x, gheight - y - 2);
              gdk_draw_line(pixmap, gc, x, gheight - y + 2, x, gheight - y + 4);
            }
          } break;
          case SATELLITE:
          case GOODS:
          ...
          default:
            gc = gcolors[players[ls->obj->player].color];
            gdk_draw_line(pixmap, gc, x, gheight - y - 2, x, gheight - y + 2);
            gdk_draw_line(pixmap, gc, x - 2, gheight - y, x + 2, gheight - y);
        }
        break;
      case ASTEROID:
        break;
    ...
    switch (ls->obj->type) {
      ...
      case ASTEROID:
        ...
        if (ls->obj->type == ASTEROID) { /* don't draw far asteroids */
          ...
          if (Distance2NearestShipLessThan(&hol, player, ls->obj->x, ls->obj->y,
                                           MAXASTEROIDDISTANCE2) == 0) {
            ls = ls->next;
            continue;
          }
        }
        gdk_draw_line(pixmap, penWhite, x, gheight - y - 2, x, gheight - y + 2);
        gdk_draw_line(pixmap, penWhite, x - 2, gheight - y, x + 2, gheight - y);
        break;
  ...
  gdk_draw_line(pixmap, penGreen, x0 - d, y0, x0, y0);
  gdk_draw_line(pixmap, penGreen, x0 - d, y0 - 2, x0 - d, y0 + 2);
  gdk_draw_line(pixmap, penGreen, x0, y0 - 2, x0, y0 + 2);
  /***** --scale *****/
  ...
  /***** target *****/
  {
    ...
    if (targetobj != NULL) {
      snprintf(point, MAXTEXTLEN, "%d %d ", (int)(distance / SECTORSIZE),
               targetobj->pid);
      xt = x0 + (targetobj->x - objx + cvx) * factor;
      yt = y0 + (targetobj->y - objy + cvy) * factor;
      gdk_draw_rectangle(pixmap, penRed, FALSE, xt - 6, gheight - yt - 6, 12,
                         12);
    } else {
      snprintf(point, MAXTEXTLEN, "%d", (int)(distance / SECTORSIZE));
    }
...
void DrawInfo(GdkPixmap* pixmap,
              Object* obj,
              struct Draw* gdraw,
              struct HeadObjList* lheadobjs,
              struct TextMessageList* lheadtext) {
  /*
  ...
  */
  ...
  if (keys.order.state == FALSE) {
    ...
    /* 	       "O: Introduce command"); */
    ShellTitle(0, NULL, pixmap, penRed, gfont, 10,
               wheight + GameParametres(GET, GPANEL, 0) / 2 + 4);
  }
  /*
   *    Ship info
   */
  ...
  if (gdraw->info == TRUE) {
    ...
    if (obj != NULL) {
      if (obj->habitat == H_PLANET) {
        planet = obj->in;
        if (planet != NULL) {
          y = DrawPlanetInfo(pixmap, gfont, penGreen, planet, obj->player, 10,
                             y);
          sprintf(point, "============");
          DrawString(pixmap, gfont, penGreen, 10, y, point);
          y += incy;
  ...
  if (PendingTextMessage()) {
    ...
    DrawMessageBox(pixmap, gfont, point, wwidth2, (int)(.25 * wheight),
                   MBOXBORDER);
  }
  ...
  lsw = 0;
  if (swgmess) {
    gdk_draw_rectangle(pixmap, penBlack, TRUE, 10,
                       wheight - incy * swgmess - 10, glen + 15,
                       incy * swgmess + 10);

    gdk_draw_rectangle(pixmap, penRed, FALSE, 10, wheight - incy * swgmess - 10,
                       glen + 15, incy * swgmess + 10);
  }
  glen = 0;
  while (lh != NULL) {
    ...
    if (lh->info.duration > 0 && i < 10) {
      ...
      */
      ...
      DrawString(pixmap, gfont, gcmessage, 20,
                 wheight - incy * swgmess + incy * i + 5, point);
      if (gfont != NULL) {
...
int DrawPlayerInfo(GdkPixmap* pixmap,
                   GdkFont* font,
                   GdkGC* color,
  ...
  */
  ...
  y += incy;
  sprintf(point, "%s:  %.0f (%.1f) %.1f", GetLocale(L_GOLD), player->gold,
          incgold0, 20 * player->balance);
  DrawString(pixmap, font, color, x0, y, point);
  y += incy;
...
int DrawPlanetInfo(GdkPixmap* pixmap,
                   GdkFont* font,
                   GdkGC* color,
                   Object* planet,
                   int player,
  ...
  */
  ...
  if (planet->player == player) {
    sprintf(point, "%s: %.1f", GetLocale(L_RESOURCES),
            planet->planet->A / 0.015);
  } else {
...
int DrawEnemyShipInfo(GdkPixmap* pixmap,
                      GdkFont* font,
                      GdkGC* color,
                      Object* obj,
                      int x0,
...
int DrawShipInfo(GdkPixmap* pixmap,
                 GdkFont* font,
                 GdkGC* color,
                 Object* obj,
                 int x0,
  ...
  */
  ...
  DrawBarBox(pixmap, color, gc, x, y, sx, texth - 1, n);
  snprintf(point, MAXTEXTLEN, "%s (Sh:%d)", GetLocale(L_STATE),
           (int)(100 * obj->shield));
  DrawString(pixmap, font, color, x + sx + 10, y + texth, point);
  y += incy;
  ...
  if (obj->cargo.capacity > 0) {
    ...
    DrawBarBox(pixmap, color, gc, x, y, sx, texth - 1, n);
    snprintf(point, MAXTEXTLEN, "%s (%d/%d)", "CARGO", obj->cargo.mass,
             obj->cargo.capacity);
    DrawString(pixmap, font, color, x + sx + 10, y + texth, point);
    y += incy;
  }
  ...
  snprintf(point, MAXTEXTLEN, "%s %.0f/%d", GetLocale(L_EXPERIENCE),
           obj->experience, (int)(100 * pow(2, obj->level)));
  DrawString(pixmap, font, color, x + sx + 10, y + texth, point);
  y += incy;
  ...
  if (obj->in != NULL && obj->in->type == SHIP) {
    snprintf(point, MAXTEXTLEN, "%s id: %d(%d) %c  L : %d", TypeCad(obj),
             obj->pid, obj->in->pid, mode, obj->level);
  } else {
    snprintf(point, MAXTEXTLEN, "%s id: %d %c  L : %d", TypeCad(obj), obj->pid,
             mode, obj->level);
  }
  ...
  if (obj->cargo.hlist != NULL) {
    ...
    if (np > 0) {
      ...
      DrawShip(pixmap, color, x0 + x + cargoobj.radio, -(y + texth) + gheight,
               &cargoobj);
      gdk_draw_rectangle(pixmap, color, FALSE, x0 + x, y + texth - texth,
                         2 * texth, 2 * texth);
      snprintf(point, MAXTEXTLEN, "%d", np);
      DrawString(pixmap, font, color, x0 + x + 2 * texth + 5,
                 y + texth - texth + 2 * texth, point);
      x += 4 * texth;
    }
    if (ns > 0) {
      cargoobj.type = SHIP;
      ...
      cargoobj.radio = 5;
      DrawShip(pixmap, color, x0 + x + 2 * cargoobj.radio,
               -(y + texth) + gheight, &cargoobj);
      gdk_draw_rectangle(pixmap, color, FALSE, x0 + x, y + texth - texth,
                         2 * texth, 2 * texth);
  ...
  snprintf(point, MAXTEXTLEN, "%s : %d (D:%d)", GetLocale(L_AMM),
           obj->weapon->n, obj->weapon->projectile.damage);
  DrawString(pixmap, font, color, x0 + x, y + texth, point);
  if (font != NULL) {
  ...
  snprintf(point, MAXTEXTLEN, "%s %d   %d", GetLocale(L_SECTOR),
  ...
  if (ord != NULL) {
    ...
    switch (ord->id) {
      case GOTO:
        ...
        if (ord->c != -1) {
          snprintf(point, MAXTEXTLEN, "%s GOTO: %d", GetLocale(L_ORDER),
                   (int)ord->e);
          dx = ox - ord->a;
          dy = oy - ord->b;
          ...
          snprintf(point, MAXTEXTLEN, "%s %s: (%d,%d)", GetLocale(L_ORDER),
                   GetLocale(L_GOTO), (int)(ord->a / SECTORSIZE) - (ord->a < 0),
        ...
        break;
      case TAKEOFF:
        snprintf(point, MAXTEXTLEN, "%s: %s", GetLocale(L_ORDER),
                 GetLocale(L_TAKEOFF));
        break;
      case ORBIT:
        snprintf(point, MAXTEXTLEN, "%s: %s", GetLocale(L_ORDER),
      ...
      case RETREAT:
        if (ord->c != -1) {
          snprintf(point, MAXTEXTLEN, "%s: %s %d", GetLocale(L_ORDER),
                   GetLocale(L_RETREAT), (int)ord->e);
          dx = ox - ord->a;
          dy = oy - ord->b;
          d = sqrt(dx * dx + dy * dy) / SECTORSIZE;
  ...
  /* --AUTO FREIGHTER */
  snprintf(point, MAXTEXTLEN, "%s: %.1f %s", GetLocale(L_DISTANCE), d,
           GetLocale(L_SECTORS));
  DrawString(pixmap, font, color, x0 + x, y + texth, point);
  y += incy;

  if (v > 2) {
    snprintf(point, MAXTEXTLEN, "%s: %.0f s", GetLocale(L_TIME),
             d * SECTORSIZE / (20 * DT * v));
  } else {
...
int XPrintTextList(GdkPixmap* pixmap,
                   GdkFont* font,
                   char* title,
                   struct HeadTextList* head,
                   int x0,
                   int y0,
                   int width,
                   int height) {
  ...
  */
  ...
  for (i = 0; i < n; i++) {
    ...
    lh = lh->next;
    y += incy;
    if (y - scroll * incy > h * incy + y0 - 5)
      break;
    if (lh == NULL)
      lh = head->next;
  }
  gdk_draw_rectangle(pixmap, penGreen, FALSE, x0, y0, textw + charw + 10,
                     height);

  return (0);
}

int XPrintMenuHead(GdkPixmap* pixmap,
                   GdkFont* font,
  ...
  while (item != NULL) {
    ...
    switch (item->type) {
      ...
      case MENUITEMTEXT:
        ...
        snprintf(point, MAXTEXTLEN, "%s %s", item->text,
...
gint SaveWindowOptions(GtkWidget* widget, gpointer gdata) {
  ...
  if ((fp = fopen((char*)gdata, "wt")) == NULL) {
    fprintf(stdout, "SaveWindowOptions(): Can't open the file: %s",
            (char*)gdata);
    exit(-1);
  }

  state = gtk_toggle_button_get_active((GtkToggleButton*)options1);
  if ((int)state != FALSE && (int)state != TRUE)
    state = FALSE;
  ...
  if (GameParametres(GET, GSOUND, 0) == TRUE &&
      GameParametres(GET, GMUSIC, 0) == TRUE) {
    Sound(SPLAY, MUSIC);
  ...
  text = gtk_entry_get_text((GtkEntry*)options5);
  value = atol(text);
  if (value < MINNUMPLANETS)
    value = MINNUMPLANETS;
  if (value > MAXNUMPLANETS)
    value = MAXNUMPLANETS;
  nplanets = value;
  ...
  if (nplayers < MINNUMPLAYERS)
    nplayers = MINNUMPLAYERS;
  if (nplayers > MAXNUMPLAYERS)
    nplayers = MAXNUMPLAYERS;
  ...
  text = gtk_entry_get_text((GtkEntry*)options18);
  value = atol(text);
  if (value < MINNUMGALAXIES)
    value = MINNUMGALAXIES;
  if (value > MAXNUMGALAXIES)
    value = MAXNUMGALAXIES;
...
void DrawPlayerList(GdkPixmap* pixmap,
                    int player,
                    struct HeadObjList* hlp,
                    Object* cvobj,
                    int act) {
  /*
  ...
  */
  ...
  if (cvobj != last_cv || act) {
    ...
    while (ls != NULL) {
      ...
      switch (obj->type) {
        case SHIP:
          if (obj->state < 50 || obj->gas < 0.5 * obj->gas_max) {
            color = 2;
          }
          if (obj->state < 25 || obj->gas < 0.25 * obj->gas_max) {
            color = 3;
          }
          if (obj->type == SHIP && obj->subtype == PILOT) {
            color = 3;
          ...
          if (obj == cvobj || obj->selected == TRUE) {
            color = 1;
          }
          ...
          if (obj->gas < 0.95 * obj->gas_max) {
            strncpy(tmpcad, cad, MAXTEXTLEN);
            snprintf(cad, MAXTEXTLEN, "%se:%d ", tmpcad,
                     (int)(100 * obj->gas / obj->gas_max));
          }
          ord = ReadOrder(NULL, obj, MAINORD);
          if (ord != NULL) {
            strncpy(tmpcad, cad, MAXTEXTLEN);
          ...
          if (obj->in != NULL) {
            ...
            if (obj->in->player != obj->player) {
              snprintf(cad, MAXTEXTLEN, "%sIN:(%d)%d", tmpcad, obj->in->player,
                       pid);
            } else {
        ...
        case PLANET:
          ...
          if (cvobj != NULL) {
            if (cvobj->player != obj->player)
              color = 4;
            if (cvobj->in == obj)
              color = 1;
          }

          snprintf(cad, MAXTEXTLEN, "id:%d g:%.0f  %d", obj->id,
                   obj->planet->gold, shipsinplanets[obj->id - 1]);
          textw = gdk_text_width(gfont, cad, strlen(cad));
          if (textw > textpw)
            textpw = textw;
          Add2TextList(&hplanetlist, cad, color);
  ...
  XPrintTextList(pixmap, gfont, titleships, &hshiplist, 10, 15,
                 textsw + charw + 10, GameParametres(GET, GHEIGHT, 0) - 50);

  XPrintTextList(pixmap, gfont, "PLANETS:", &hplanetlist,
                 GameParametres(GET, GWIDTH, 0) - textpw - 20, 15,
                 textpw + charw + 10, GameParametres(GET, GHEIGHT, 0) - 50);
...
void DrawSelectionBox(GdkPixmap* pixmap,
                      GdkGC* color,
                      int view,
                      Space reg,
                      Object* cv) {
  ...
  gdk_draw_rectangle(pixmap, penGreen, FALSE, rect.x, rect.y, rect.width,
                     rect.height);
}
...
int DrawGameStatistics(GdkPixmap* pixmap, struct Player* pl) {
  ...
  */
  ...
  gdk_draw_rectangle(pixmap, penBlack, TRUE, x - 10, y, textwidth + 20,
                     textheight * (nplayers) + 4);

  gdk_draw_rectangle(pixmap, penGreen, FALSE, x - 10, y, textwidth + 20,
                     textheight * (nplayers) + 4);
  ...
  /* HERE: send kills and deaths to client */
  {
    ...
    for (i = 0; i < nplayers - 1; i++) {
      if (pl[a[i]].status >= PLAYERACTIVE) { /* HERE set default name */
        snprintf(cad, MAXTEXTLEN,
                 "%d: %s, [L%d-%d : %d : %1.1f], %s: %d, %s: %d (%d), %s: %d, "
                 "%s: %d.",
                 i + 1, pl[a[i]].playername, pl[a[i]].gmaxlevel,
                 pl[a[i]].maxlevel, pl[a[i]].level + pl[a[i]].nships,
                 (float)(pl[a[i]].level) / (pl[a[i]].nships + 0.0001),
                 GetLocale(L_PLANETS), pl[a[i]].nplanets, GetLocale(L_SHIPS),
                 pl[a[i]].nships, pl[a[i]].nbuildships, GetLocale(L_KILLS),
                 pl[a[i]].nkills, GetLocale(L_DEATHS), pl[a[i]].ndeaths);

      } else if (pl[a[i]].status == PLAYERDEAD) {
        snprintf(cad, MAXTEXTLEN,
                 "%d: %s, [L%d] GAMEOVER, %s: %d (%d), %s: %d, %s: %d, %s: %d.",
                 i + 1,
                 /* GetLocale(L_dPLAYER), */
                 pl[a[i]].playername, pl[a[i]].gmaxlevel, GetLocale(L_SHIPS),
                 pl[a[i]].nships, pl[a[i]].nbuildships, GetLocale(L_PLANETS),
                 pl[a[i]].nplanets, GetLocale(L_KILLS), pl[a[i]].nkills,
      ...
      DrawString(pixmap, gfont, gcolors[players[a[i]].color], x,
                 y + (i + 2) * textheight, cad);
    }
    free(a);
  }
  return (x - 10);
...
void DrawString(GdkDrawable* pixmap,
                GdkFont* font,
                GdkGC* gc,
                gint x,
                gint y,
...
void DrawMessageBox(GdkPixmap* pixmap,
                    GdkFont* font,
                    char* cad,
                    int x0,
                    int y0,
  ...
  */
  ...
  gdk_draw_rectangle(pixmap, penBlack, TRUE, x, y, width, height);
  switch (type) {
    case MBOXDEFAULT:
      break;
    case MBOXBORDER:
      gdk_draw_rectangle(pixmap, penGreen, FALSE, x, y, width, height);
      break;
    default:
      break;
  }
...
void DrawCharList(GdkPixmap* pixmap,
                  GdkFont* font,
                  GdkGC* color,
  ...
  */
  ...
  if (swgmess) {
    gdk_draw_rectangle(pixmap, penBlack, TRUE, x0, y0, glen + 25,
                       incy * (swgmess + 1));

    gdk_draw_rectangle(pixmap, penRed, FALSE, x0, y0, glen + 25,
                       incy * (swgmess + 1));

    gdk_draw_line(pixmap, penRed, x0 + glen + 10, y0, x0 + glen + 10,
                  y0 + incy * (swgmess + 1));
  }
...
void DrawBarBox(GdkPixmap* pixmap,
                GdkGC* border,
                GdkGC* color,
                int x,
                int y,
...
void DrawWindow(GdkPixmap* pixmap,
                GdkFont* font,
                GdkGC* color0,
                int x0,
                int y0,
  ...
  switch (type) {
    ...
    case 1: /* window with scroll */
      ...
      if (swgmess) {
        ...
        gdk_draw_rectangle(pixmap, penBlack, TRUE, window.x, window.y,
                           window.width, window.height);

        gdk_draw_rectangle(pixmap, penRed, FALSE, window.x, window.y,
                           window.width, window.height);
        ...
        /* 		    y0+window.height); */
        {
          ...
          /* cursor */
          for (i = 0; i <= 5; i++) {
            gdk_draw_line(pixmap, penRed, xbar, ybar + y0 - 5 + i * 2,
                          xbar + wbar, ybar + y0 - 5 + i * 2);
          }
          /* arrows */
          gdk_draw_line(pixmap, penRed, xbar + 2, y0 + 8, xbar + wbar / 2,
                        y0 + 2);
          gdk_draw_line(pixmap, penRed, xbar + wbar / 2, y0 + 2,
                        xbar + wbar - 2, y0 + 8);

          gdk_draw_line(pixmap, penRed, xbar + 2, y0 + window.height - 8,
                        xbar + wbar / 2, y0 + window.height);
          gdk_draw_line(pixmap, penRed, xbar + wbar / 2, y0 + window.height,
      ...
      while (list != NULL) {
        ...
        DrawString(pixmap, font, color, x0 + 5, y0 + incy * (swgmess + 1) + 5,
                   point);
        textw = gdk_text_width(font, point, strlen(point));
        if (textw > glen)
          glen = textw;
...
void DrawStatistics(GdkPixmap* pixmap,
                    Rectangle* r,
                    struct Stats* stats,
                    int stats_n,
                    int stats_np) {
  ...
  for (i = 0; i < stats_np; i++) {
    for (j = 0; j < NSTATS; j++) {
      if (stats[j].level[i] > maxlevel)
        maxlevel = stats[j].level[i];
      if (stats[j].nplanets[i] > maxnplanets)
        maxnplanets = stats[j].nplanets[i];
    }
    if (players[i].level + players[i].nships > maxlevel)
      maxlevel = players[i].level + players[i].nships;
    if (players[i].nplanets > maxnplanets)
      maxnplanets = players[i].nplanets;
  }
  ...
  if ((USEPIXMAPS && !(cont % 1)) || !USEPIXMAPS) {
    ...
    gdk_draw_rectangle(pm, penBlack, TRUE, rect.x, rect.y, rect.width,
                       rect.height);

    gdk_draw_rectangle(pm, penGreen, FALSE, rect.x, rect.y, rect.width - 1,
                       rect.height - 1);
    ...
    for (i = 1; i < stats_np; i++) {
      ...
      for (j = 0; j < stats_n - 1; j++) {
        ...
        gdk_draw_line(pm, gcolors[players[i].color], x0, y0, x1, y1);
        x0 = x1;
        y0 = y1;
      }
      x1 = rect.x + (j + 1) * dx;
      y1 = rect.y + (rect.height - 1) -
           (players[i].level + players[i].nships) * dy;
      gdk_draw_line(pm, gcolors[players[i].color], x0, y0, x1, y1);
    }
  }
  ...
  if (USEPIXMAPS) {
    ...
    rect.width = r->width;
    rect.height = r->height;
    gdk_draw_drawable(pixmap, gc_stat, pixmap_stat, 0, 0, rect.x, rect.y,
