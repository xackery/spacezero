...
int Shell(int command,
  ...
  */
  ...
  if (lhead == NULL || pixmap == NULL || color == NULL || ps == NULL ||
      key == NULL) {
    level = 0;
    return (0);
  }
  ...
  if (level == 0) {
    key->g = key->x = key->s = key->p = key->t = key->o = key->r = key->b =
        key->u = key->e = key->d = FALSE;
    /* aqui  */
    strcpy(cad, "");
    strcpy(ord, "");
    strcpy(par, "");
  ...
  if (level == 1) {
    if (*pcv != NULL) {
      strncpy(cad,
              "G:GOTO   X:EXPLORE   S:SELECT   P:STOP   T:TAKEOFF   O:ORBIT   "
              "R:REPEAT   B:BUY   U:UPGRADE   E:SELL   D:RETREAT",
              MAXTEXTLEN);
  ...
  if (level == 2) {
    switch (order) {
        ...
        if (key->mright == TRUE && *pcv != NULL) {
          ...
          switch (view) {
            case VIEW_MAP:
              ...
              nearobj =
                  ObjNearThan(lhead, player, a, b, 500000); /* 0.5 sectors */
              if (nearobj != NULL)
                pid = nearobj->pid;
              else
                pid = 0;
          ...
          switch (nargs) {
            ...
            case 2:
              ...
              sprintf(par, "%d %d", id1 * SECTORSIZE + SECTORSIZE / 2,
                      id2 * SECTORSIZE + SECTORSIZE / 2);
              break;
            default:
              printf("ERROR (%d)\n", nargs);
              break;
      ...
      case BUY:
        strcpy(cad, "");
        if ((*pcv)->type == SHIP && (*pcv)->subtype == PILOT) {
          snprintf(cad, MAXTEXTLEN, "                  2: FIGTHER(%d)",
                   GetPrice(NULL, FIGHTER, ENGINE4, CANNON4));
        } else {
          snprintf(cad, MAXTEXTLEN,
                   "1: EXPLORER(%d)   2: FIGTHER(%d)   3: TOWER(%d)   4: "
                   "FREIGHTER(%d)   5: SATELLITE(%d)",
      ...
      case SELL:
        ...
        if (cv->mode == LANDED) {
          ...
          snprintf(cad, MAXTEXTLEN, "DESTROYING %s%s", par,
                   "   (Esc to cancel)");
        }
        break;
      case WRITE:
  ...
  if (key->enter == TRUE) {
    ...
    if (order == GOTO || order == TAKEOFF || order == ORBIT ||
        order == EXPLORE || order == STOP || order == RETREAT) {
...
Object* ExecOrder(struct HeadObjList* lhead,
                  Object* obj,
                  struct Player* ps,
                  int order,
                  char* par) {
  ...
  */
  ...
  switch (order) {
    ...
    case GOTO:
      ...
      switch (nargs) {
        case 1: /* GOTO a planet or a ship */
          ...
          switch (arg1[0]) {
            ...
            case 'N':
              ...
              obj_dest = NearestObj(lhead, obj, PLANET, SHIP0, PINEXPLORE,
                                    &d2); /* HERE only one function */
            ...
            switch (obj_dest->type) {
              case PLANET: /* if planet is unknown*/
                if (IsInIntList((ps->kplanets), id1) == 0) {
                  printf("%s. %d %s.\n", GetLocale(L_NOTALLOWED), obj_dest->pid,
                  ...
                  printf("(%c %d) %s %d.\n", Type(obj), obj->pid,
                         GetLocale(L_GOINGTOPLANET), obj_dest->pid);
                  snprintf(stmess, MAXTEXTLEN, "(%c %d) %s %d.", Type(obj),
                           obj->pid, GetLocale(L_GOINGTOPLANET), obj_dest->pid);
                  ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
                }
                break;
              case SHIP: /* ship belongs to another player*/
                if (obj_dest->player != obj->player) {
                  ...
                  snprintf(stmess, MAXTEXTLEN,
                           "%s. Destiny is an enemy spaceship.",
                           GetLocale(L_NOTALLOWED));
                  ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
                } else {
                  if (obj_dest == obj) {
                    ...
                    snprintf(stmess, MAXTEXTLEN, "(%c %d)  %s.", Type(obj),
                             obj->pid,
                             /* GetLocale(L_NOTALLOWED), */
                             GetLocale(L_DESTINYEQUALORIGIN));
                    ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
                    ...
                    printf("(%c %d) %s %d.\n", Type(obj), obj->pid,
                           GetLocale(L_GOINGTOSPACESHIP), obj_dest->pid);
                    snprintf(stmess, MAXTEXTLEN, "(%c %d) %s %d.", Type(obj),
                             obj->pid, GetLocale(L_GOINGTOSPACESHIP),
                             obj_dest->pid);
                    ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
                  }
                }
        ...
        case 2: /* GOTO sector    AQUI */
          ...
          snprintf(stmess, MAXTEXTLEN, "(%c %d) %s %d %d.", Type(obj), obj->pid,
                   GetLocale(L_GOINGTOSECTOR),
                   (id1 - SECTORSIZE / 2) / SECTORSIZE,
                   (id2 - SECTORSIZE / 2) / SECTORSIZE);
          ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
    ...
    case EXPLORE:
      ...
      DelAllOrder(obj);
      AddOrder(obj, &ord);
      printf("(%c %d) %s.\n", Type(obj), obj->pid, GetLocale(L_GOINGTOEXPLORE));
      snprintf(stmess, MAXTEXTLEN, "(%c %d) %s.", Type(obj), obj->pid,
    ...
    case SELECT:
      ...
      if (obj_dest != NULL) {
        ...
        if (obj_dest != NULL) {
          ...
          snprintf(stmess, MAXTEXTLEN, "(%c %d) %s.", Type(obj_dest),
                   obj_dest->pid, GetLocale(L_SELECTED));
          ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
        } else {
        ...
        printf("%s. %s.\n", GetLocale(L_NOTALLOWED),
               GetLocale(L_PLANETORSPACESHIPUNKNOWN));
        snprintf(stmess, MAXTEXTLEN, "%s. (%s) %s.", GetLocale(L_NOTALLOWED),
                 arg1, GetLocale(L_PLANETORSPACESHIPUNKNOWN));
        ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
        /* keys.esc=TRUE; */
        ret = NULL;
      }
          ...
          switch (id1) {
            case 1: /* EXPLORER */
              ...
              if (status == SZ_OK) {
                ...
                snprintf(stmess, MAXTEXTLEN, "%s %s.", GetLocale(L_EXPLORER),
                         GetLocale(L_BUYED));
                ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
              }
              break;
            case 2: /* FIGHTER */
              ...
              if (status == SZ_OK) {
                ...
                snprintf(stmess, MAXTEXTLEN, "%s %s.", GetLocale(L_FIGHTER),
                         GetLocale(L_BUYED));
                ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
              }
              break;
            case 3: /* TOWER */
              ...
              if (status == SZ_OK) {
                ...
                snprintf(stmess, MAXTEXTLEN, "%s %s.", GetLocale(L_TOWER),
                         GetLocale(L_BUYED));
                ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
              }
              break;
            ...
            case 5: /* SATELLITE */
              ...
              if (status == SZ_OK) {
                ...
                snprintf(stmess, MAXTEXTLEN, "%s %s.", GetLocale(L_SATELLITE),
                         GetLocale(L_BUYED));
                ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
              }
              break;
    ...
    case SELL:
      if (obj->mode == LANDED) {
        printf("%s: %d\n", GetLocale(L_SELLINGSPACESHIP), obj->pid);
        snprintf(stmess, MAXTEXTLEN, "%s: %d", GetLocale(L_SELLINGSPACESHIP),
                 obj->pid);
        ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
        price = .5 * GetPrice(obj, 0, 0, 0);
        if (price > 0) {
          printf("\tPrice: %d eng: %d weapon: %d\n", price, obj->engine.type,
                 obj->weapon->type);
      ...
      if (obj->level + 1 < ps->gmaxlevel) {
        ...
        if (price > 0) {
          if (ps->gold > price) {
            ...
            Experience(obj,
                       (int)(100 * pow(2, obj->level) - obj->experience + 1));
            printf("(%c %d) %s %d.\n", Type(obj), obj->pid,
                   GetLocale(L_UPGRADETOLEVEL), obj->level);
            snprintf(stmess, MAXTEXTLEN, "(%c %d) %s %d.", Type(obj), obj->pid,
                     GetLocale(L_UPGRADETOLEVEL), obj->level);
            ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
          } else {
            printf("%s\n", GetLocale(L_YOUHAVENOTGOLD));
        ...
        printf("%s %d\n", GetLocale(L_YOUCANUPGRADE), ps->gmaxlevel - 1);
        snprintf(stmess, MAXTEXTLEN, "%s %d", GetLocale(L_YOUCANUPGRADE),
                 ps->gmaxlevel - 1);
        ShellTitle(1, stmess, NULL, NULL, NULL, 0, 0);
        ret = NULL;
      }
      ...
      if (mouserelease) { /* mouse release */
        ...
        if (region.rect.width == 0 || region.rect.height == 0) {
          ...
          Window2Real(cv, view, region.rect.x + region.rect.width,
                      region.rect.y + region.rect.height, &x1, &y1);
          ...
          if (n > 0) {
            snprintf(stmess, MAXTEXTLEN, "%d %s.", n,
                     GetLocale(L_SPACESHIPSSELECTED));
