...
void Usage(char* ver, char* l_rev) {
  ...
  (void)fprintf(stdout,
                "          spacezero [GENERAL OPTIONS] -s [MULTIPLAYER "
                "OPTIONS] [GAME OPTIONS]\n");
  (void)fprintf(
      stdout,
      "          spacezero [GENERAL OPTIONS] -c [MULTIPLAYER OPTIONS]\n");
  (void)fprintf(stdout, "          spacezero -h\n");
  (void)fprintf(stdout, "\n-s\t\t server mode (multiplayer).\n");
  (void)fprintf(stdout, "-c\t\t client mode (multiplayer).\n");
  (void)fprintf(stdout, "-h\t\t this help.\n");
  (void)fprintf(stdout, "\nGENERAL OPTIONS:\n");
  (void)fprintf(
      stdout, "-soundvol n\t where n is the sound effects volume. (0..100)\n");
  (void)fprintf(stdout,
  ...
  (void)fprintf(
      stdout, "-name playername where playername is the name of the player.\n");
  (void)fprintf(stdout, "-font font\t change the default font by font.\n");
  (void)fprintf(stdout, "-nomenu \t Starts automatically, without menu.\n");
  (void)fprintf(stdout,
                "-lang [en,es] \t Game language. English(default) and spanish "
                "supported.\n");
  (void)fprintf(stdout, "\nGAME OPTIONS:\n");
  (void)fprintf(stdout,
  ...
  (void)fprintf(stdout, "-pirates\t Add a team of pirates (default).\n");
  (void)fprintf(stdout, "-nopirates\t Don't add a team of pirates.\n");
  (void)fprintf(stdout, "-enemyknown\t All enemies are known.\n");
  (void)fprintf(stdout, "-noenemyknown\t All enemies are unknown (default).\n");
  (void)fprintf(stdout, "\nMULTIPLAYER OPTIONS:\n");
  (void)fprintf(stdout,
  ...
  (void)fprintf(
      ...
      DEFAULT_PORT);
  (void)fprintf(stdout, "\nKeyboard controls:\n==================\n");
  (void)fprintf(stdout, "up arrow\t accelerate/manual mode.\n");
  (void)fprintf(stdout, "left-right arrows turn left-right/manual mode.\n");
  ...
  (void)fprintf(stdout, "f7\t\t show game messages log.\n");
  (void)fprintf(stdout, "o\t\t enter in order menu.\n");
  (void)fprintf(stdout,
                "Esc\t\t cancel actual order.\n\t\t close info windows.\n");
  (void)fprintf(stdout, "m\t\t show space map.\n");
  (void)fprintf(stdout, "up/down arrows\t manual/automatic mode.\n");

  (void)fprintf(stdout, "Ctrl-w\t\t Write a meesage to other players.\n");
  (void)fprintf(stdout, "Ctrl +/-\t volume up/down.\n");

  (void)fprintf(stdout, "Ctrl-n\t\t window, ship mode view.\n");
  (void)fprintf(stdout, "Ctrl-p\t\t pause game\n");
  ...
  (void)fprintf(stdout, "space\t\t center map on current ship.\n");
  (void)fprintf(stdout, "mouse pointer\t show coordinates.\n");
  (void)fprintf(stdout, "l\t\t show-hide labels.\n");
  (void)fprintf(stdout, "left mouse \t Select the nearest ship.\nbutton\n");
  (void)fprintf(
      stdout,
  ...
  (void)fprintf(stdout, "\nlast revision: %s\n", l_rev);
  (void)fprintf(stdout,
                "Please, send bugs and suggestions to: mrevenga at "
...
void PrintWarnings(char* version) {
  ...
  fprintf(
      stderr,
      "WARNING: This is the development version of SpaceZero (version %s)\nIt "
      "can contain bugs.\nNet and saved games can be incompatible with the "
      "official release.\nMaybe SpaceZero doesn't work properly.\n",
      version);

  if (sizeof(int) != 4) {
    fprintf(stderr,
            "\nWARNING: size of int %d bytes\n, maybe SpaceZero doesn't work "
            "properly\n",
            (int)sizeof(int)); /* HERE warning: format '%d' expects type 'int',
                                  but argument 3 has type 'long unsigned int' */
  }
  fprintf(stderr,
          "\nWARNING: Communication between 64 and 32 bits machines not so "
          "tested,\nmaybe SpaceZero doesn't work properly.\n");
