...
void aimissile(struct HeadObjList* lhobjs, Object* obj) {
  ...
  if (ship_enemy != NULL) {
    int sw = 0;
    if (ship_enemy->habitat != obj->habitat)
      sw = 1;
    if (ship_enemy->type == PLANET && obj->habitat == H_PLANET)
      sw = 1;
    if (sw)
      ship_enemy = NULL;
  }
  ...
  switch (obj->level) {
    ...
    default:
      ...
      d2 = (x2 - obj->x) * (x2 - obj->x) + (y2 - obj->y) * (y2 - obj->y);
      t1 = sqrt(d2 / obj->engine.v2_max / 2);
      x2 = ship_enemy->x + (ship_enemy->vx - obj->vx) * t1;
      y2 = ship_enemy->y + (ship_enemy->vy - obj->vy) * t1;
...
void ai(struct HeadObjList* lhobjs, Object* obj, int act_player) {
  ...
  */
  ...
  if (obj->cdata->obj[0] != NULL ||
      (time - obj->cdata->td2[0]) > 50 + obj->id % 20) {
    for (i = 0; i < 4; i++) {
      nobjs[i].obj = NULL;
      nobjs[i].d2 = -1;
    }
    ...
    if (nobjs[0].obj != NULL) {
      ...
      if (nobjs[0].obj->type == ASTEROID &&
          control ==
              COMPUTER) { /* HERE send to asteroids from ControlCenter() */
  ...
  if (ship_enemy != NULL && (int)obj->player == act_player && cv != obj) {
    ...
    switch (ship_enemy->type) {
      case SHIP:
        if (obj->habitat == H_PLANET &&
            (players[obj->player].team == players[obj->in->player].team)) {
          objt = obj->in;
          if (ship_enemy->habitat == H_SPACE) {
            snprintf(text, MAXTEXTLEN, "(P %d) %s", objt->pid,
                     GetLocale(L_ENEMIESNEAR));
            value = 1;
          } else {
            snprintf(text, MAXTEXTLEN, "%s %d %s!!", GetLocale(L_PLANET),
                     objt->pid, GetLocale(L_UNDERATTACK));
            value = 3;
          }
        } else {
    ...
    if (!Add2TextMessageList(&listheadtext, text, objt->id, objt->player, 0,
                             100, value)) {
  ...
  /* Automatic trade */
  if (obj->type == SHIP && obj->subtype == FREIGHTER && ship_enemy == NULL) {
    if (obj->mode == LANDED && obj->cargo.mass == obj->cargo.capacity) {
      if (obj->destid != obj->oriid && obj->destid != 0 && obj->oriid != 0) {
        Object* obj_dest;
  ...
  ********************/
  ...
  if (mainord != NULL) {
    if (mainord->id ==
        RETREAT) { /* in retreat, when reach destiny change the order */
      int sw = 0;
      if ((int)mainord->c != -1) {
        if (obj->habitat == H_PLANET) {
          if (obj->in->id == (int)mainord->c) { /* dest reached */
            if (obj->in->player != obj->player)
              sw = 1;
            if (obj->in->player == obj->player || obj->in->player == 0)
              sw = 2;
  ...
  if (obj->actorder.id == -1 && obj->norder < 10) {
    ...
    switch (danger) {
      case 0: /* no danger, execute main order */
        if (mainord != NULL) {
          ...
          if (mainord->id == EXPLORE &&
              (int)mainord->h == 1) { /* select sector */
            int a, b;
          ...
          switch (mainord->id) {
            ...
            case EXPLORE:
              ...
              if ((int)mainord->c != -1) { /* dest is an object */
                objt =
                    Coordinates(lhobjs, mainord->c, &mainord->a, &mainord->b);
              ...
              if (objt != NULL) {
                ...
                if (objt == obj)
                  order.id = STOP; /* PILOT RESCUED  */
                if (objt->type == PLANET) {
                  if (obj->habitat == H_PLANET) {
                    if (obj->in->id == objt->id) {
                      if ((int)obj->player == act_player &&
                          (int)mainord->h == 0) {
                        snprintf(text, MAXTEXTLEN, "(%c %d) %s %d", Type(obj),
                                 obj->pid, GetLocale(L_ARRIVEDTO), obj->in->id);
                        if (!Add2TextMessageList(&listheadtext, text, obj->id,
                                                 obj->player, 0, 100, 0)) {
                          Add2CharListWindow(&gameloglist, text, 1,
                                             &windowgamelog);
                        }
                      }
                      mainord->h = 1;
              ...
              if ((int)mainord->c == -1) { /* destination is a sector */
                if (fabs(obj->x - mainord->a) < 400 &&
                    fabs(obj->y - mainord->b) < 400) {
                  if ((int)obj->player == act_player && (int)mainord->h == 0 &&
                      mainord->id != EXPLORE) {
                    snprintf(
                        text, MAXTEXTLEN, "(%c %d) %s %d %d", Type(obj),
                        obj->pid, GetLocale(L_ARRIVEDTO),
      ...
      case 1: /* enemy near */
        if (ship_enemy != NULL) {
          ...
          if (obj->habitat == H_PLANET) {
            ...
            if (obj->mode != LANDED && mainordid == STOP) {
              order.id = LAND;
            }

            if (ship_enemy->habitat == H_SPACE && mainordid != STOP) {
              if (obj->weapon0.n > 0.5 * obj->weapon0.max_n &&
                  obj->gas > .50 * obj->gas_max && obj->state > 50) {
                if (obj->engine.a_max) {
                  order.id = TAKEOFF;
                }
              } else {
    ...
    switch (order.id) {
      ...
      case NOTHING:
        ...
        obj->accel = 0;
        order.time = 20;
        if (obj->mode == NAV && obj->habitat == H_PLANET)
          order.time = 1;
    ...
    if (execord->time >= 0) {
      switch (execord->id) {
        ...
        case ORBIT: {
          ...
          planet = NearestObj(lhobjs, obj, PLANET, SHIP0,
                              POWN | PALLY | PINEXPLORE | PENEMY, &d2);
          if (planet == NULL) {
            DelAllOrder(obj);
            order.id = NOTHING;
            break;
...
int CreateCCData(void) {
  ...
  if (np0 == 0) {
    ccdatap = malloc((GameParametres(GET, GNPLAYERS, 0) + 2) *
                     sizeof(struct CCDATA)); /* +1 system +1 pirates*/
    if (ccdatap == NULL) {
      fprintf(stderr, "ERROR in malloc (ccdatap)\n");
      exit(-1);
    }
    MemUsed(MADD,
            (GameParametres(GET, GNPLAYERS, 0) + 2) * sizeof(struct CCDATA));
    np0 = np;
  }

  if (np0 != np) {
    ccdatap = realloc(ccdatap, (GameParametres(GET, GNPLAYERS, 0) + 2) *
                                   sizeof(struct CCDATA));
    if (ccdatap == NULL) {
  ...
  for (i = 0; i < GameParametres(GET, GNPLAYERS, 0) + 2;
       i++) { /* HERE TODO include ccdata in player*/
    ccdatap[i].player = i;
...
Object* CCUpgrade(struct HeadObjList* lhobjs, struct Player* player) {
  ...
  while (ls != NULL) {
    ...
    if (obj->level >= player->gmaxlevel - 1) {
      ls = ls->next;
      continue;
    }
...
int CCBuy(struct HeadObjList* lhobjs,
  ...
  if (player->id !=
      ccdata->planethighresource->player) { /* HERE this happens (maybe if in
                                               the meanwhile change of owner) */
  ...
  /* max rel. fighter/tower 1 */
  if ((float)(ccdata->nfighter + ccdata->nexplorer / 2) /
          (float)ccdata->ntower <
      0.3) {
    *planetid = ccdata->planetweak->id;
    return (FIGHTER);
  ...
  */
  ...
  while (ls != NULL && ordersw == 0) {
    ...
    if (obj->state < 25 && actord->id != RETREAT) {
      ...
      obj_dest =
          NearestObj(lhobjs, obj, PLANET, SHIP0, PINEXPLORE | PALLY, &d2);
    ...
    switch (actord->id) {
        ...
        switch (obj->mode) {
          case LANDED:
            ...
            if (shipsinplanet[ls->obj->in->id] == -1) {
              shipsinplanet[ls->obj->in->id] = CountShipsInPlanet(
                  NULL, ls->obj->in->id, ls->obj->player, SHIP, -1, 2);
            }
            /* don't send if it is alone */
            if (shipsinplanet[ls->obj->in->id] < 2)
              break;
            ...
            if (ccdata->war && obj->subtype == FIGHTER) {
              switch (ccdata->war) {
                ...
                case 3:
                  if (obj->habitat == H_PLANET &&
                      obj->in->id == ccdata->planet2meet->id) {
          ...
          case NAV: /* actual order=NOTHING */
            ...
            if (obj->gas > 0.25 * obj->gas_max &&
                obj->cdata->a == 0) { /* return or explore*/
              naplanet = NearestObj(lhobjs, obj, PLANET, SHIP0, PALLY,
                                    &d2); /* double loop */
              niplanet = NearestObj(lhobjs, obj, PLANET, SHIP0, PINEXPLORE,
                                    &d2); /* double loop */

              neplanet = NULL;
              if (naplanet == NULL && niplanet == NULL) {
                neplanet = NearestObj(lhobjs, obj, PLANET, SHIP0, PENEMY,
                                      &d2); /* double loop */
              }
              nplanet = NULL;
              if (naplanet != NULL) {
      ...
      case EXPLORE:
        sw = 0;
        if (obj->x > maxx || obj->x < -maxx || obj->y > maxy ||
            obj->y < -maxy) {
        ...
        if (obj->cdata->obj[2] != NULL &&
            obj->cdata->d2[2] < 2 * obj->radar * obj->radar) {
        ...
        if (sw) {
          ...
          if (nplanet == NULL) {
            nplanet = NearestObj(lhobjs, obj, PLANET, SHIP0, PALLY,
                                 &d2); /* double loop */
            if (nplanet == NULL) {
              nplanet = NearestObj(lhobjs, obj, PLANET, SHIP0, PINEXPLORE,
                                   &d2); /* double loop */
            }
          }
          if (nplanet != NULL) {
          ...
          switch (ls->obj->subtype) {
            ...
            case FIGHTER:
              ...
              if ((int)ls->obj->level <= level) {
                ...
                if (obj->level == 0 && (int)obj->experience == 0 && n > 1)
                  return (ls->obj);
              }
              break;
            default:
...
int Risk_08(struct HeadObjList* lhobjs, Object* obj) {
  ...
  */
  ...
  /******** no danger *************/
  if (ship_enemy == NULL && obj->gas > .75 * obj->gas_max && obj->state > 50 &&
  ...
  if (obj->weapon0.n + obj->weapon1.n + obj->weapon2.n < 20) {
    action[2] *= 1.2;
  }
  if (obj->state < 25) {
    action[2] *= 1.5;
  ...
  */
  ...
  for (j = 0; j < 3; j++) {
    ...
    switch (weapon->projectile.type) {
      ...
      case MISSILE: /*SHOT3: */
      case LASER:   /*SHOT4 */
        if (obj->dest_r2 < a[0][j])
          b[0][j] = 1;
        if (weapon->n > a[1][j] * weapon->max_n)
          b[1][j] = 1;
        if (obj->gas > a[2][j])
          b[2][j] = 1;
        if (weapon->cont1 != 0)
...
int FireCannon(struct HeadObjList* lhobjs, Object* obj1, Object* obj2) {
  ...
  if (OtherProc(lhobjs, GetProc(), obj1) == 0)
    enemies = 0;
  ...
  switch (obj1->weapon->type) {
    ...
    case CANNON7: /* not finished, not used */
      ...
      for (i = 0; i < n; i++) {
        ...
        obj = NewObj(PROJECTILE, obj1->weapon->projectile.type,
                     obj1->x + r * cos(ang), obj1->y + r * sin(ang),
                     vp * cos(ang), vp * sin(ang), CANNON0, ENGINE0,
                     obj1->player, obj1, obj1->in);
        if (obj != NULL) {
          if (obj1->habitat == H_PLANET) {
            obj->y += obj1->radio;
            obj->y0 += obj1->radio;
    ...
    case CANNON8: /* missile */
      ...
      obj = NewObj(PROJECTILE, obj1->weapon->projectile.type,
                   obj1->x + r * cos(ang), obj1->y + r * sin(ang),
                   vp * cos(ang) + obj1->vx, vp * sin(ang) + obj1->vy, CANNON0,
                   ENGINE2, obj1->player, obj1, obj1->in);
      if (obj != NULL) {
        if (obj1->habitat == H_PLANET) {
          obj->y += obj1->radio;
          obj->y0 += obj1->radio;
    ...
    case CANNON9: /* laser */
      ...
      obj = NewObj(PROJECTILE, obj1->weapon->projectile.type,
                   obj1->x + r * cos(ang), obj1->y + r * sin(ang),
                   vp * cos(ang) + obj1->vx, vp * sin(ang) + obj1->vy, CANNON0,
                   ENGINE0, obj1->player, obj1, obj1->in);
      if (obj != NULL) {
        if (obj1->habitat == H_PLANET) {
          obj->y += obj1->radio;
          obj->y0 += obj1->radio;
...
void CreatePirates(struct HeadObjList* lhobjs,
                   int n,
                   float x0,
                   float y0,
                   float level) {
  ...
  if (GameParametres(GET, GPIRATES, 0) == FALSE)
    return;
  players = GetPlayers();
  for (i = 0; i < n; i++) {
    x = 1024 * Random(-1);
    ...
    if (i == 0) { /* EXPLORER*/
      obj = NewObj(SHIP, EXPLORER, x0 + x, y0 + y, 0, 0, CANNON3, ENGINE3,
                   GameParametres(GET, GNPLAYERS, 0) + 1, NULL, NULL);
    } else { /* FIGHTER*/
      obj = NewObj(SHIP, FIGHTER, x0 + x, y0 + y, 0, 0, CANNON4, ENGINE4,
                   GameParametres(GET, GNPLAYERS, 0) + 1, NULL, NULL);
    }
...
void CreateAsteroids(struct HeadObjList* lhobjs, int n, float x0, float y0) {
  ...
  */
  ...
  for (i = 0; i < n; i++) {
    ...
    obj = NewObj(ASTEROID, size + 1, x0 + x, y + y0, vx, vy, CANNON0, ENGINE0,
                 0, NULL, NULL);
  ...
  */
  ...
  while (ks != NULL) {
    if (p1->team == p2->team) {
      ...
      /* if are enemies */
      if ((((obj->level + 0.5) / 3.) * gnplanets) * ((float)rand() / RAND_MAX) <
          1) {
        if (!IsInIntList(p1->kplanets, ks->id)) {
          p1->kplanets = Add2IntList(p1->kplanets, ks->id);
          snprintf(text, MAXTEXTLEN, "%s", GetLocale(L_RECEIVEDINFO));

          if (!Add2TextMessageList(&listheadtext, text, obj->id, obj->player, 0,
                                   100, 0)) {
...
int OtherProc(struct HeadObjList* lh, int p, Object* obj0) {
  ...
  */
  ...
  while (ls != NULL) {
    ...
    if (players[obj1->player].proc == p) {
      ls = ls->next;
      continue;
    }

    if (obj1->type != SHIP && obj1->type != ASTEROID) {
      ls = ls->next;
      continue;
    }
...
void CalcCCInfo(struct HeadObjList* lhobjs,
                struct HeadObjList* lhkplanets,
                int player,
                struct CCDATA* ccdata) {
  /*
    version 01
  ...
  while (ls != NULL) { /* known planets */
    ...
    if (ccdata->player == planet->player)
      ccdata->nplanets++;
    if (planet->player == 0)
      ccdata->ninexplore++;
    else {
  ...
  while (ls != NULL) {
    ...
    /* only add ally ships */
    if (players[obj->player].team != players[player].team) {
      ls = ls->next;
      continue;
    }
    ...
    if (obj->habitat == H_PLANET) {
      if (pinfo->planet->id == obj->in->id) {
        ...
        if (ccdata->player == obj->player) {
          if (obj->level >=
              obj->in->level - 2) { /* don't include learning ships*/
            float factor = 0;
            factor = (obj->level) * (obj->state) / 100 * (obj->state > 75) *
                     (obj->gas >= .80 * obj->gas_max) *
                     (obj->weapon0.n > .8 * obj->weapon0.max_n) *
                     (obj->mode == LANDED);
            if (obj->subtype == FIGHTER || obj->subtype == QUEEN ||
                obj->subtype == EXPLORER) {
              factor *=
                  (obj->state > 95) * (obj->gas > .98 * obj->gas_max) *
                  (obj->weapon0.n >= .95 * obj->weapon0.max_n) *
                  (obj->level >= players[pinfo->planet->player].gmaxlevel - 2);
...
void CalcCCPlanetStrength(int player, struct CCDATA* ccdata) {
  ...
  while (pinfo != NULL) { /* known planets */
    if (pinfo->planet->player != player) {
      pinfo = pinfo->next;
      continue;
    }

    if (pinfo->ntower + pinfo->nfighter + pinfo->nexplorer + pinfo->ncargo >
        0) {
      if (sw == 0) {
        ...
        float resources =
            pinfo->planet->planet->reggold / 0.015 - pinfo->ntower;
    ...
    switch (info) {
      case CCDATAPRINT:
        printf("planet: %d ne:%d nf:%d nt:%d\n", pinfo->planet->id,
               pinfo->nexplorer, pinfo->nfighter, pinfo->ntower);
        break;
      case CCDATANSHIPS:
        return (pinfo->nexplorer + pinfo->nfighter + pinfo->ntower +
                pinfo->ncargo);
        break;
...
void PrintCCPlanetInfo(struct CCDATA* ccdata) {
  ...
  while (pinfo != NULL) {
    printf("planet: %d ne:%d nf:%d nt:%d na:%d\n", pinfo->planet->id,
           pinfo->nexplorer, pinfo->nfighter, pinfo->ntower, pinfo->nassigned);
...
int NearestCCPlanets(struct CCDATA* ccdata,
                     Object* obj,
                     int status,
  ...
  while (pinfo != NULL) {
    switch (status) {
      case PENEMY:
        ...
        if (players[pinfo->planet->player].team == players[player].team) {
          pinfo = pinfo->next;
          continue;
        }
        break;
      case PINEXPLORE:
        ...
        if (pinfo->planet->player != 0 && pinfo->planet->player != player) {
          pinfo = pinfo->next;
          continue;
        }
        nships =
            pinfo->nexplorer + pinfo->nfighter + pinfo->ntower + pinfo->ncargo;
        if (pinfo->planet->player == player && nships > 0) {
          pinfo = pinfo->next;
          continue;
        }
        /* only send one ship */
        if (pinfo->nassigned > 0) {
          pinfo = pinfo->next;
          continue;
        }
        break;
  ...
  switch (strategy) {
    ...
    case PLAYERSTRATDEFAULT: /* from nearest to nearest*/
      ...
      while (pinfo != NULL) { /* double loop */
        ...
        pinfo1 = pinfo->next;
        while (pinfo1 != NULL) {
          if (pinfo1->planet->player == 0) {
            pinfo1 = pinfo1->next;
            continue;
          }
          if (pinfo->planet->player == pinfo1->planet->player) {
            pinfo1 = pinfo1->next;
            continue;
          }
          ...
          if (team1 != team0 && team2 != team0) {
            pinfo1 = pinfo1->next;
            continue;
          }

          if (sw == 0) { /* first time */
            d2min = (pinfo->planet->x - pinfo1->planet->x) *
                        (pinfo->planet->x - pinfo1->planet->x) +
                    (pinfo->planet->y - pinfo1->planet->y) *
                        (pinfo->planet->y - pinfo1->planet->y);
            if (team0 == team1) {
          ...
          d2 = (pinfo->planet->x - pinfo1->planet->x) *
                   (pinfo->planet->x - pinfo1->planet->x) +
               (pinfo->planet->y - pinfo1->planet->y) *
                   (pinfo->planet->y - pinfo1->planet->y);
    ...
    case PLAYERSTRATSTRONG: /* from strongest to nearest */
      ...
      while (pinfo != NULL) {
        ...
        if (player->id != pinfo->planet->player) {
          pinfo = pinfo->next;
          continue;
        }
        ...
        if (pinfo->strengtha > strength) {
          ...
          if (pinfo->strengtha == strength) {
            if (100 * Random(-1) <
                20) { /* HERE must depend on number of planets*/
              planet = pinfo->planet;
            }
          }
        }
    ...
    case PLAYERSTRATWEAK: /* attack weakest */
      ...
      if (planet != NULL) {
        ...
        ccdata->planet2meet =
            GetNearPlanet(ccdata, ccdata->planet2attack, PALLY);
      }
      break;
    default:
      break;
  ...
  switch (player->profile) {
    case PLAYERPROFDEFAULT:
      ...
      if (ccdata->nplanets < 10) {
        if (ccdata->nkplanets > 5 && ccdata->nfighter > 10 && ftower > 2.8 &&
            ccdata->ninexplore == 0) {
          ret = 1; /* entering phase 1 */
        }
      } else { /* with more planets */
        if (ccdata->nkplanets > 10 && ccdata->nfighter > 10 && ffighter > 1 &&
            ftower > 2.8 && ccdata->ninexplore == 0) {
          ret = 1; /* entering phase 1 */
        }
      }

      /* if resources are needed */
      if (ccdata->nfighter > 10 && ccdata->ninexplore == 0 &&
          player->balance < 0 && ftower >= 3) {
        ret = 1;
      }
    ...
    case PLAYERPROFAGRESSIVE:
      ...
      /* with few planets */
      if (ccdata->nplanets < 10) {
        if (ccdata->nkplanets > 5 && ccdata->nfighter > 8 && ftower > 2.0 &&
            ccdata->ninexplore == 0) {
          ret = 1; /* entering phase 1 */
        }
      } else { /* with more planets */
        if (ccdata->nkplanets > 10 && ccdata->nfighter > 10 && ffighter > 0.7 &&
            ftower > 2.0 && ccdata->ninexplore == 0) {
          ret = 1; /* entering phase 1 */
        }
      }
  ...
  while (pinfo != NULL) {
    switch (pstate) {
      case POWN:
        ...
        if (ccdata->player != pinfo->planet->player) {
          pinfo = pinfo->next;
          continue;
        }
        break;
    ...
    if (sw == 0) { /* first time */
      ...
      d2min =
          (pinfo->planet->x - planet1->x) * (pinfo->planet->x - planet1->x) +
...
struct PlanetInfo* CalcEnemyPlanetInfo(struct HeadObjList* lhobjs,
                                       struct CCDATA* ccdata,
                                       Object* obj) {
  /*
  ...
  */
  ...
  if (retpinfo != NULL) {
    ...
    while (ls != NULL) {
      ...
      if ((players[ls->obj->player].team == players[ccdata->player].team)) {
        ls = ls->next;
        continue;
      }
      if (ls->obj->type != SHIP) {
...
int UpdateEnemyPlanetInfo(struct HeadObjList* lhobjs,
                          struct CCDATA* ccdata,
                          int player) {
  /*
    ...
    if (players[player].team != players[pinfo->planet->player].team) {
      if (GetTime() - pinfo->time > 7000) { /* 7000 */
        ...
        if (players[pinfo->planet->player].nships > 0) {
          smin = (3 + pinfo->planet->mass / 50000) *
                 pow(2, (float)players[pinfo->planet->player].level /
                            players[pinfo->planet->player].nships);
        } else {
          smin = 0;
        }
        pinfo->strength -= (int)(pinfo->strength / 2);
        if (pinfo->strength < smin)
          pinfo->strength = smin;
        pinfo->time = GetTime();
        ret++;
      }
...
int CountAssignedCCPlanetInfo(struct HeadObjList* lhobjs,
                              struct CCDATA* ccdata,
                              Object* planet) {
  /*
  ...
  */
  ...
  while (ls != NULL) {
    ...
    if ((players[obj->player].team == players[ccdata->player].team)) {
      ls = ls->next;
      continue;
    }
    if (obj->type != SHIP) {
    ...
    if (order->id == GOTO && (int)order->c != planet->id)
      n++;
...
struct PlanetInfo* War(struct HeadObjList* lhobjs,
                       struct Player* player,
                       struct CCDATA* ccdata) {
  /*
    returns :
...
*/
  ...
  if (ccdata->war >= 2) {
    ...
    if (players[ccdata->planet2attack->player].team ==
        players[player->id].team) { /* planet conquered */
    ...
    if (players[ccdata->planet2meet->player].team !=
        players[player->id].team) { /* origin planet lost */
  ...
  switch (ccdata->war) {
    ...
    case 1:
      ...
      if (WarCCPlanets(player, ccdata) !=
          1) { /* HERE TODO review this for differents values of 1 */
...
int BuyorUpgrade(struct HeadObjList* lhobjs,
                 struct Player* player,
                 struct CCDATA* ccdata) {
  /*
...
*/
  ...
  np = (player->nplanets);
  if (np == 0)
    return (0); /* HERE this doesn't allow to pilots to buy when there are not
                   planets */
          ...
          if (ccdata->war) {
            if (ccdata->planet2meet != NULL) {
              if (player->id == ccdata->planet2meet->player) {
                ...
                if (ccdata->planethighlevel != NULL) {
                  if (ccdata->planethighlevel->level >
                      ccdata->planet2meet->level) {
                    planetid = ccdata->planethighlevel->id;
                  }
                }
              }
      ...
      if (obj2up == NULL) {
        ...
        if (price > 0) {
          if (player->gold > price) {
            ...
            Experience(obj2up, (int)(100 * pow(2, obj2up->level) -
                                     obj2up->experience + 1));
            player->lastaction = 0;
            ret = 2;
          }
...
void ExecLand(Object* obj, struct Order* ord) {
  ...
  x0 = ord->a + obj->radio +
       (ord->b - 2. * obj->radio) * (obj->pid % 40) / 40.; /* Landing point */

  dx = obj->x - x0;

  if (dx > LXFACTOR / 2 || dx < -LXFACTOR / 2)
    dx *= -1;
  ...
  swvx = 0;
  if (dx > 0) {
    if (obj->vx > -vxmax && obj->vx < -vxmax + 1) {
      a = -b;
    } /* OK */
    if (obj->vx < -vxmax) {
      a = -b;
  ...
  if (dx < 0) {
    if (obj->vx > vxmax - 1 && obj->vx < vxmax) {
      a = b;
    } /* OK */
    if (obj->vx < vxmax - 1) {
      a = -b;
...
void ExecAttack(struct HeadObjList* lhobjs,
                Object* obj,
                struct Order* ord,
                struct Order* morder,
                float d2) {
  ...
  */
  ...
  if (obj->dest->in != obj->in &&
      obj->mode == LANDED) { /* HERE check this condition */
    ExecStop(obj, 0, 0);
    return;
  }
  ...
  if (morder != NULL) {
    switch (morder->id) {
      case STOP:
        ...
        break;
      case RETREAT: /* HERE goto to point and attack if is in the right
                       direction */
  ...
  if (shot) {
    ...
    alcance2 =
        .5 * obj->weapon->projectile.max_vel * obj->weapon->projectile.life;
    alcance2 *= alcance2;
  ...
  if (obj->habitat == H_PLANET && obj->engine.a_max && obj->mode != LANDED) {
    ...
    if (obj->y < ord->b + (.35 + .10 * obj->in->mass / MAXPLANETMASS) *
                              LYFACTOR) { /* refered to higher planet point */
      ord->time = 0;
    ...
    case SHIP:
      ...
      switch (obj->habitat) {
        ...
        case H_SPACE:
          ...
          if (fabs(ab) < .1 &&
              obj->engine.v2_max - (obj->vx * obj->vx + obj->vy * obj->vy) <
                  0.025 * obj->engine.v2_max) { /* FIX in 0.84 */
            obj->accel = 0;
            swaccel = 0;
            return;
          } else {
...
int ExecOrbit(Object* obj, Object* planet, struct Order* mainord) {
  ...
  h2 = 1000 / (MAXPLANETMASS - MINPLANETMASS) * planet->mass + 3200 -
       (1000 / (MAXPLANETMASS - MINPLANETMASS) * MAXPLANETMASS);
  if (obj->actorder.b > 50)
    h2 = obj->actorder.b * obj->actorder.b;
  e = 0.5;
  if (obj->actorder.c > 0 && obj->actorder.c < 1.5)
    e = obj->actorder.c;
  s = 1;
...
int ExecTurnAccel(Object* obj, float c, float prec) {
  ...
  int ret = 1;
  /*
    obj turns accelerating until the velocity reaches the angle c with a
    precision prec also the obj must reach their maximum velocity .
...
int ExecStop(Object* obj, float v0, float iv2) {
  ...
  if (iv2 < 0.01)
    iv2 = 0.01; /* limit to reach */
  if (v0 < 0.01)
    v0 = 0.01; /* limit to reach */
