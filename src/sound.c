...
char* soundnames[NUM_SOUNDS] = {"music.ogg", "bfire.ogg", "explos.ogg",
                                "thrust.ogg", "crash.ogg"};
...
ALvoid* LoadMemoryFromOggFile(char* fname,
                              ALenum* format,
                              ALsizei* size,
                              ALfloat* frequency);
...
int InitSound(void) {
  ...
  *****/
  ...
  datadir = DATADIR;
  snprintf(filesoundnames[0], MAXTEXTLEN, "%s/sounds/%s", datadir,
           soundnames[0]);
  ...
  for (i = 0; i < NUM_SOUNDS; i++) {
    snprintf(filesoundnames[i], MAXTEXTLEN, "%s/sounds/%s", datadir,
             soundnames[i]);
    printf("\tsound(%d): %s\n", i, filesoundnames[i]);
  }
...
int PlaySound(int sid, int mode, float vol) {
  ...
  if (bufferid == -1) {
    ...
    /* load the buffer */
    data =
        LoadMemoryFromOggFile(filesoundnames[sid], &format, &size, &frequency);
    if (data == NULL) {
  ...
  for (i = 0; i < NUM_SOURCES; i++) {
    alGetSourcei(sources[i], AL_BUFFER,
                 &sourceState); /*returns the buffer id
...
int CleanBuffers(void) {
  ...
  for (i = 0; i < NUM_BUFFERS; i++) {
    ...
    for (j = 0; j < NUM_SOURCES; j++) {
      alGetSourcei(sources[j], AL_BUFFER,
                   &sourceState); /*returns the buffer id
                                  0 if is not a buffer associated*/
